...
{
   uint64_t lo = getBits(source, low, high);
   lo = lo | 0xff;
   lo = lo << low;
   if (low == 0 && high == 63)
   {
...
{
    uint64_t lo = getBits(source, srclow, srclow + length - 1);
    lo = lo << dstlow;
    uint64_t lon = clearBits(dest, dstlow, dstlow + length - 1);
    if (dstlow >= 63 && length >= 2)
    {
        return dest;
    }
   return lo | lon;
...
{
// for some reason, this doesn't work for 0 on the student machine. It'll
// run the solution fine on any simulator or program.
    uint64_t lo = source << ((63 - (byteNum + 1) * 8) - 1);
    lo = lo >> ((63 - (byteNum + 1) * 8) - 1);
    lo = lo >> (byteNum * 8);
    lo = lo | 0xff;
    lo = lo << (byteNum * 8);
    lo = lo | source;
...
{
    uint64_t sup = op1 + op2;
    uint64_t val = getBits(sup, 59, 63);
    if (val > 15)
    {
        return false;
    }
...
{
    uint64_t sup = op2 - op1;
    uint64_t sal = getBits(sup, 59, 63);
    if (sal > 0)
    {
        return true;
    }
